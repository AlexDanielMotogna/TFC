...
import type { Trade } from '@/hooks/useUserTrades';
...
export function PerformanceChart({ fights, trades, userId, mode }: PerformanceChartProps) {
  ...
  }, [fights, timeRange, mode]);

  // Filter and sort trades by date
  const filteredTrades = useMemo(() => {
    if (mode !== 'trades' || !trades) return [];
    ...
    return trades
      .filter((t) => {
        // Apply time range filter
        if (timeRange !== 'all') {
          const executedAt = new Date(t.executedAt).getTime();
          const daysAgo = timeRange === '7d' ? 7 : 30;
          if (now - executedAt > daysAgo * dayMs) return false;
        }

        return true;
      })
      .sort((a, b) => new Date(a.executedAt).getTime() - new Date(b.executedAt).getTime());
  }, [trades, timeRange, mode]);
  ...
  const cumulativeData = useMemo(() => {
    ...
    if (mode === 'fights') {
      ...
      return filteredTrades.map((trade, index) => {
        ...
        return {
          index: index + 1,
          date: new Date(trade.executedAt).toLocaleDateString('en-US', {
            month: 'short',
            day: 'numeric',
          }),
          pnl: parseFloat(cumulative.toFixed(2)),
  ...
  }, [finishedFights, filteredTrades, userId, mode]);

  // Process data for individual fights/trades chart
  ...
  // Process data for win rate trend (rolling 10-fight/trade average)
  const winRateData = useMemo(() => {
    ...
    if (mode === 'fights') {
      ...
      // Trades mode - profit rate
      return filteredTrades.map((trade, index) => {
        const startIndex = Math.max(0, index - windowSize + 1);
        const window = filteredTrades.slice(startIndex, index + 1);

        const profitable = window.filter((t) => {
          const pnl = t.pnl ? parseFloat(t.pnl) : 0;
          return pnl > 0;
        }).length;
        const profitRate = window.length > 0 ? (profitable / window.length) * 100 : 0;
        ...
        return {
          ...
          winRate: parseFloat(profitRate.toFixed(1)),
          tradeId: trade.id,
        };
      });
    }
  ...
  const xAxisInterval = useMemo(() => {
    const dataLength = mode === 'fights' ? finishedFights.length : filteredTrades.length;
  ...
  }, [finishedFights.length, filteredTrades.length, mode]);

  // Determine if we should show dots (only for small datasets)
  const showDots = (mode === 'fights' ? finishedFights.length : filteredTrades.length) <= 30;
  ...
  const hasData = mode === 'fights' ? finishedFights.length > 0 : filteredTrades.length > 0;
          ...
          <p className="text-surface-500">
            {mode === 'fights'
              ? timeRange === 'all'
                ...
                : `No fights in the last ${timeRange === '7d' ? '7 days' : '30 days'}`
              : timeRange === 'all'
              ? 'No individual trades yet'
              : `No trades in the last ${timeRange === '7d' ? '7 days' : '30 days'}`}
          </p>
        </div>
      </div>
    );
