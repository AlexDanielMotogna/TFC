...
/**
 * Proxy for Nado Gateway queries.
 * The Nado API doesn't support CORS, so browser-side code (WS adapter)
...
export async function POST(req: NextRequest) {
  try {
    ...
    // --- Proxy to Nado Gateway ---
    const body = await req.json();

    const response = await fetch(`${NADO_GATEWAY_URL}/query`, {
      method: 'POST',
      headers: {
    ...
    console.error('[NadoProxy] Gateway query error:', error);
