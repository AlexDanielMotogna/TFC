 ...
 *
 * Centralized module for recording trades to the Trade and FightTrade tables.
 * Extracted from orders/route.ts for reuse by:
 * - POST /api/orders (market orders)
 * - POST /api/internal/record-fill (limit/stop order fills detected by fill-detector)
 *
 ...
 */
export async function emitStakeInfo(
  fightId: string,
  userId: string,
  stake: number,
 ...
 */
export async function recordAllTrades(
...
/**
 * Record ALL trades with pre-fetched execution details.
 * Used by the fill detector when it already has trade history data from Pacifica.
 * Skips the retry loop for fetching trade history since details are provided.
 */
export async function recordAllTradesWithDetails(
  accountAddress: string,
  symbol: string,
  side: string,
  amount: string,
  ...
  if (!connection) {
    console.log('[recordAllTradesWithDetails] No user found for account:', accountAddress);
    return;
  }
  ...
  // Save to Trade table using pre-fetched details
  try {
    const trade = await prisma.trade.create({
      data: {
        userId: connection.userId,
        ...
        side: tradeSide,
        amount,
        price: execDetails.executionPrice,
        fee: execDetails.fee,
        pnl: execDetails.pnl,
        leverage: leverage || null,
        fightId: shouldAssignFightId ? resolvedFightId : null,
    ...
    broadcastAdminTrade({
      ...
      side: tradeSide,
      amount: parseFloat(amount),
      price: parseFloat(execDetails.executionPrice),
    ...
    // Calculate referral commissions (non-blocking)
    const priceNum = parseFloat(execDetails.executionPrice);
    const amountNum = parseFloat(amount);
    const tradeValue = priceNum * amountNum;
    const tfcFee = tradeValue * TFC_FEE_RATE;
    ...
    if (err.code !== 'P2002') {
      console.error('[recordAllTradesWithDetails] Failed to save trade:', err);
    }
  }
 ...
 */
export async function recordFightTradeWithDetails(
  accountAddress: string,
  symbol: string,
  side: string,
  amount: string,
