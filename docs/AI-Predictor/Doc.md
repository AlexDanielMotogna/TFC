...
AI Signal is a built-in trading assistant powered by Claude AI. It analyzes real-time market data across multiple timeframes (1H, 4H, 1D) and provides actionable trading signals directly inside the TFC trading terminal.

### How to use it

1. **Open the widget** — Click the AI Signal orb on the right side of the trade page
2. **Select your risk profile** — Choose between Safe (conservative), Balanced (moderate), or Aggro (aggressive)
3. **Click "Analyze"** — The AI analyzes the current market in real time
4. **Review the signal** — You'll see LONG, SHORT, or STAY OUT with entry, stop loss, and take profit levels
...
- **FACTORS** — Shows the key factors the AI considered (trend, momentum, volume, etc.) with bullish/bearish/neutral bias for each
- **POS** — Shows AI advice for your open positions (HOLD, CLOSE, ADD, REDUCE, MOVE SL). Only appears when you have positions in the current market
...
| **MOVE SL** | Move your stop loss to a new level (e.g., to lock in profits) |
