...
|---------|---------------|--------|
| One fight per user | `fights.service.ts`, `fights/route.ts`, `fights/[id]/join/route.ts` | ✅ Done |
| Consolidated exposure calculation | New: `fight-exposure.ts`, Updated: `orders.ts`, `stake-info/route.ts` | ✅ Done |
| Atomic maxExposureUsed update | `fight-exposure.ts`, `orders/route.ts` | ✅ Done |
| RULE 35 API failure handling | `orders/route.ts` | ✅ Done |
| Navbar realized PnL only | `FightBanner.tsx` (already was realized-only backend) | ✅ Done |
| Open position warning | `FightBanner.tsx` | ✅ Done |
| External trades at end only | `fight-engine.ts` | ✅ Done |
| Unit tests | `fight-exposure.test.ts` | ✅ Done |

### New Files Created

- `apps/web/src/lib/server/fight-exposure.ts` - Centralized exposure calculation utilities
- `apps/web/src/lib/server/fight-exposure.test.ts` - Unit tests for exposure calculation
...
### 1. One Fight Per User (MVP-1)

**Files Modified:**
- `apps/api/src/modules/fights/fights.service.ts`
- `apps/web/src/app/api/fights/route.ts`
- `apps/web/src/app/api/fights/[id]/join/route.ts`

**Logic:**
```typescript
// Check for existing LIVE or WAITING fights before create/join
const existingFight = await prisma.fightParticipant.findFirst({
  where: {
    userId,
    fight: { status: { in: ['LIVE', 'WAITING'] } },
  },
});

if (existingFight) {
  throw new Error('You already have an active/pending fight');
}
```

### 2. Consolidated Exposure Calculation

**New File:** `apps/web/src/lib/server/fight-exposure.ts`

**Exports:**
- `calculateFightExposure(fightId, userId)` - Calculate exposure from FightTrade records
- `calculateExposureFromTrades(trades)` - Pure function for testing
- `calculateAvailableCapital(stake, maxExposureUsed, currentExposure)` - Standard formula
- `updateMaxExposureIfHigher(participantId, newExposure)` - Atomic DB update
- `hasActiveFight(userId)` - Check for active fight
- `hasWaitingFight(userId)` - Check for pending fight

**Atomic maxExposureUsed Update:**
```sql
UPDATE fight_participants
SET max_exposure_used = GREATEST(max_exposure_used, $newExposure)
WHERE id = $participantId
```

This prevents race conditions when multiple trades execute concurrently.

### 3. RULE 35 API Failure Handling

**File:** `apps/web/src/app/api/orders/route.ts`

**Problem:** When Pacifica `getPositions()` failed, the code continued with `pacificaPositionAfter = 0`, causing incorrect trade classification.

**Fix:** Use TFC net position as fallback (conservative approach - records full trade as fight-relevant).

### 4. Navbar Realized PnL (MVP-5)

**File:** `apps/web/src/components/FightBanner.tsx`

**Note:** Backend already calculated realized-only PnL. Added tooltips to clarify.

### 5. Open Position Warning (MVP-6)

**File:** `apps/web/src/components/FightBanner.tsx`

Uses `useFightPositions` hook to detect open positions and show warning badge.

### 6. External Trade Check at End Only (MVP-9)

**File:** `apps/realtime/src/fight-engine.ts`

**Removed:**
```typescript
// Previously checked every 30 seconds during tick loop
if (shouldCheckExternalTrades && fight.startedAt) {
  await this.checkExternalTrades(fight);
}
```

**Kept:** Final check in `endFight()` method (already existed).
...
```bash
# Fight exposure tests
cd apps/web
npx ts-node src/lib/server/fight-exposure.test.ts

# PnL calculation tests
cd apps/realtime
npx ts-node src/fight-pnl-calculator.test.ts
```
...
- [ ] Create fight when user has active fight → Should fail
- [ ] Join fight when user has active fight → Should fail
- [ ] Create fight when user has pending fight → Should fail
- [ ] Open position during fight → Warning shows in banner
- [ ] Close position → PnL updates in navbar
- [ ] Fight ends → External trades checked once
- [ ] Multiple concurrent orders → maxExposureUsed updates atomically
- [ ] Pacifica API fails during trade → Falls back to TFC net position
